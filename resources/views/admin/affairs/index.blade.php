                  ...
                  <td style="word-break: break-all; white-space: break-spaces;">{{$value->heading}}</td>
                    ...
                    <div class="row">
                      <div class="col-sm-6" align="right">
    										<a href="{{url('/admin/affairs/'.$value->id)}}" class="btn btn-warning" title="Edit Course" style="color: white; font-size:15px; font-weight: 600; margin:2px;">
    											<i class="fa fa-edit" style="font-size:16px"></i> Edit
    										</a>
                        </div>
                      <div class="col-sm-6" align="left">
                        <form action="{{url('/admin/affairs/delete/'.$value->id)}}" method="POST" role="form" id="delform{{$value->id}}">
                          {!! csrf_field() !!}
					...
					<div class="card-footer clearfix">
						@if(count($affairs) > 0)
						{{$affairs->appends(['search' => $search, 'limit' => $limit])->links()}}
						@endif
					</div>
				</div>
			</div>
		</div>
		<a href="{{url('/admin/affairs/create')}}" title="Add New Affairs">
  ...
  <input type="hidden" id="delId">
	...
	<script type="text/javascript">   
		...
		$('#searchBtn').click(function() {
			var url = '{{url('/admin/affairs')}}?search=' + $('#search').val() + '&limit=' + $('#limit').val();
