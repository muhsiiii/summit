	...
	<div class="content-wrapper">
		...
		<div class="row m-10">
			<div class="col-sm-3 mb-10">
				<input type="text" class="form-control" id="search" placeholder="Search Something..." value="{{$search}}">
			</div>
                  ...
                  <td style="word-break: break-all; white-space: break-spaces;">{{$value->heading}}</td>
                    ...
                    <div class="row">
                      <div class="col-sm-6" align="right">
    										<a href="{{url('/admin/articles/'.$value->id)}}" class="btn btn-warning" title="Edit Course" style="color: white; font-size:15px; font-weight: 600; margin:2px;">
    											<i class="fa fa-edit" style="font-size:16px"></i> Edit
    										</a>
                        </div>
                      <div class="col-sm-6" align="left">
                        <form action="{{url('/admin/articles/delete/'.$value->id)}}" method="POST" role="form" id="delform{{$value->id}}">
                          {!! csrf_field() !!}
					...
					<div class="card-footer clearfix">
						@if(count($articles) > 0)
						{{$articles->appends(['search' => $search, 'limit' => $limit])->links()}}
						@endif
					</div>
				</div>
			</div>
		</div>
		<a href="{{url('/admin/articles/create')}}" title="Add New Article">
  ...
  <input type="hidden" id="delId">
	...
	<script type="text/javascript">   
		...
		$('#searchBtn').click(function() {
			var url = '{{url('/admin/articles')}}?search=' + $('#search').val() + '&limit=' + $('#limit').val();
